...
进程上下文切换不是只保存几个寄存器。内核通常需要保存当前进程的 CPU 寄存器、程序计数器、栈指针、内核栈状态，并切换到另一个进程的地址空间和内核调度状态。

开销主要来自三部分。

- **CPU 状态保存与恢复** ：寄存器、栈、程序计数器等需要切换。
- **地址空间切换** ：不同进程页表不同，可能导致 TLB 失效或命中率下降。
- **缓存局部性破坏** ：新进程的数据不一定在 CPU cache 中。

线程切换通常比进程切换轻，因为同一进程内的线程共享地址空间，页表不需要切换。但线程切换仍然有寄存器、栈和调度开销。

## Linux CFS 调度器的基本思想是什么？

CFS 的目标是让可运行任务尽量公平地共享 CPU。它使用虚拟运行时间 `vruntime` 衡量任务已经获得的 CPU 时间，优先调度 `vruntime` 较小的任务。

`nice` 值会影响任务权重。权重越高，虚拟运行时间增长越慢，因此更容易获得 CPU。CFS 并不是简单的时间片轮转，而是通过红黑树维护可运行任务，并让调度结果接近“每个任务按权重公平分配 CPU”的理想模型。

面试里可以这样落点：CFS 关注公平性和交互响应，实时任务不走普通 CFS 规则；CPU 密集任务和 I/O 密集任务的调度表现也会不同。

## fork 之后父子进程的内存关系是什么？

`fork` 后父子进程拥有独立的虚拟地址空间，但很多虚拟页一开始映射到相同物理页，并通过写时复制共享。只读代码段天然可以共享；可写数据页会在任一方写入时复制。

这解释了两个现象。

- `fork` 很快，因为它主要复制页表和进程元数据，而不是复制全部物理内存。
- 子进程修改变量不会影响父进程，因为写入时会分裂出新的物理页。

如果 `fork` 后马上 `exec`，COW 可以避免大量无意义复制；如果 `fork` 后父子都写大量内存，COW 的优势会下降。
...
![匿名管道](../Figures/Operating%20System/anonymous-pipe.png)
...
![共享内存](../Figures/Operating%20System/shared-memory.png)
...
![进程状态转换](../Figures/Operating%20System/process-state-transition.png)
