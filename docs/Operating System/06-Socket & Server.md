...
LT 是 level-triggered，表示只要文件描述符仍然处于可读或可写状态，`epoll_wait` 就会持续返回该事件。它更接近 select / poll 的语义，写法简单，不容易漏事件。

ET 是 edge-triggered，只在状态从不可读变为可读、不可写变为可写时通知一次。使用 ET 时必须配合非阻塞 fd，并在一次通知后循环读到 `EAGAIN` 或写到 `EAGAIN`，否则缓冲区中剩余数据可能不会再次触发通知，连接就像“卡住”一样。

面试时可以总结：LT 更稳，ET 更高效但更容易写错；高性能服务器使用 ET 时，关键是非阻塞和一次性 drain。

## epoll 惊群问题如何理解？EPOLLONESHOT 有什么用？

惊群指多个线程或进程同时等待同一批事件时，一个连接到来可能唤醒多个执行流，但最终只有少数能真正处理事件，其他执行流被白白唤醒，造成调度开销。

`EPOLLONESHOT` 的作用是某个 fd 的事件被一个线程拿到后，内核临时禁用该 fd 上的事件通知，避免多个线程同时处理同一个连接。处理完成后需要通过 `epoll_ctl` 重新注册或修改事件，才能继续接收该 fd 的后续事件。

实践中还会结合多 Reactor、连接分片、`SO_REUSEPORT` 等方式降低竞争。

## 零拷贝是不是完全没有拷贝？

不是。零拷贝通常指减少 CPU 参与的数据拷贝，尤其是减少“内核缓冲区到用户缓冲区再回到内核”的往返拷贝。数据从磁盘到网卡仍然可能经过 DMA、页缓存、socket 缓冲区等路径。

例如 `sendfile` 可以让文件数据从页缓存直接发送到 socket，避免用户态缓冲区拷贝和用户态 / 内核态多次切换。`splice` 则可以在管道和 fd 之间移动数据。

答题时要说清边界：零拷贝优化的是数据路径和 CPU 拷贝次数，不等于没有内存访问，也不等于所有场景都更快。小文件、动态压缩、加密、业务需要改写响应体时，传统读写可能更合适。
...
- **4 次用户态与内核态的上下文切换** ：因为发生了两次系统调用，一次是 `read()`，一次是 `write()`，每次系统调用都得先从用户态切换到内核态，等内核完成任务后，再从内核态切换回用户态
