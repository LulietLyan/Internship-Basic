...
虚拟内存的核心不是“把磁盘当内存用”这么简单，而是给每个进程提供独立、连续、受保护的虚拟地址空间。进程看到的是虚拟地址，CPU 通过页表和 TLB 将其翻译成物理地址。

它主要解决三个问题。

- **地址隔离** ：不同进程的虚拟地址可以相同，但映射到不同物理页，避免进程互相破坏内存。
- **按需分配** ：申请虚拟地址空间不一定马上分配物理页，只有真正访问时才可能触发缺页并分配。
- **扩展能力** ：通过换页、文件映射、共享页等机制，让内存管理更灵活。

面试追问时可以补充：虚拟内存依赖页表完成地址转换，TLB 缓存热点页表项；TLB 未命中不等于缺页，缺页是页表项无有效物理映射或权限不满足时触发的异常。
...
1. CPU 触发 page fault，切换到内核态。
2. 内核检查访问地址是否合法，例如是否落在进程的 VMA 区间内。
3. 如果地址非法或权限不匹配，向进程发送 `SIGSEGV`。
4. 如果合法但物理页不存在，内核分配物理页，或从磁盘 / 文件加载页面。
5. 更新页表和必要的 TLB 状态，返回用户态重新执行触发异常的指令。

常见追问是 minor fault 和 major fault 的区别：minor fault 通常不需要磁盘 I/O，例如分配匿名页或复用已在内存中的页；major fault 需要从磁盘读取页面，开销明显更高。

## 为什么 malloc 申请了很大内存，进程的物理内存不一定马上上涨？

`malloc` 本质上向用户态分配器申请一段可用地址空间，大块内存可能进一步通过 `mmap` 向内核申请虚拟地址区域。这个过程常常只是建立虚拟地址范围，并不会立刻分配等量物理页。

真正触碰页面，例如写入数组元素时，才会触发缺页异常并分配物理页。这也是 Linux 上 overcommit 能成立的原因之一：系统允许承诺的虚拟内存超过实际物理内存，但如果后续大量页面同时被真实访问，就可能触发 OOM。

面试回答可以强调两个指标。

- `VIRT` 表示虚拟地址空间大小，不等于实际占用内存。
- `RES` / RSS 更接近当前驻留在物理内存中的页。

## mmap 和普通 read / write 有什么区别？

`read` / `write` 是显式系统调用，数据通常经历“内核页缓存 ↔ 用户缓冲区”的拷贝；`mmap` 将文件内容映射到进程虚拟地址空间，用户态像访问内存一样读写文件页，缺页时由内核把对应文件页调入内存。

`mmap` 的优势是减少显式拷贝，适合随机访问、大文件局部访问、多个进程共享同一文件映射等场景。但它不是任何时候都更快：大量顺序读写、小文件、访问模式不可控时，`read` / `write` 的简单顺序预读可能更稳定。

还要区分两个常见映射模式。

- `MAP_SHARED` ：写入会反映到映射文件，对其他共享映射进程可见。
- `MAP_PRIVATE` ：写入触发写时复制，不会回写到原文件。

## 什么是写时复制？

写时复制，简称 COW，是一种延迟复制策略。`fork` 创建子进程时，父子进程最初共享相同物理页，页表项被标记为只读；任一进程尝试写共享页时触发缺页异常，内核再复制一份物理页，并让写入方映射到新页。

这样做能显著降低 `fork` 成本。典型场景是 `fork` 后立刻 `exec`，子进程很快加载新程序，如果一开始就完整复制父进程内存，会浪费大量时间和内存。

需要注意：COW 降低的是“未写页面”的复制成本。如果父子进程随后都大量写内存，仍然会产生大量页面复制。

## 什么是内存抖动？如何定位和治理？

内存抖动指系统频繁换入换出页面，CPU 大量时间消耗在缺页和磁盘 I/O 上，业务吞吐反而下降。它通常出现在工作集明显大于可用内存、并发过高、缓存无限增长或内存泄漏场景。

定位时可以观察 major fault、swap in / out、RSS、页缓存、磁盘 I/O 等指标。如果业务延迟突然升高，同时 major fault 和 swap 活动明显增加，就要怀疑内存抖动。

治理思路包括降低并发、限制缓存容量、修复内存泄漏、增加内存、减少不必要的大对象拷贝，以及避免把延迟敏感服务部署在频繁 swap 的环境中。
