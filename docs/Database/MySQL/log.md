...
- **文件格式不同**：redo log 是物理日志，记录的是在某个数据页做了什么修改，比如对 XXX 表空间中的 YYY 数据页 ZZZ 偏移量的地方做了 AAA 更新；binlog 是逻辑日志，有三种格式：**statement** 记录原始 SQL 语句，**row** 记录每行数据的实际变更内容，**mixed** 是两者的混合，由 MySQL 自动判断使用哪种格式
