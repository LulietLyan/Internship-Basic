...
## change buffer 的作用是什么？

change buffer 是 InnoDB 针对非唯一二级索引页不在 Buffer Pool 中时的一种写优化。更新二级索引时，如果目标索引页不在内存中，InnoDB 可以先把变更记录到 change buffer，等未来读取该索引页或后台合并时再真正应用。

它的收益是减少随机读 I/O，把多次零散更新合并成更少的页读取和写入。适合写多读少、普通二级索引较多的场景。

唯一索引通常不能很好使用 change buffer，因为写入前必须检查唯一性，仍然需要把目标索引页读入内存判断是否冲突。这个点常被追问，用来区分普通索引和唯一索引在写入成本上的差异。

## 为什么写多读少的表不一定适合建很多二级索引？

每个二级索引都是一棵需要维护的 B+ 树。写入、更新、删除数据时，不仅要改聚簇索引，还要维护相关二级索引页。索引越多，写放大越明显，Buffer Pool 压力和 redo log 量也会增加。

如果某个索引查询收益很低，却频繁参与写入维护，就可能拖慢整体吞吐。优化时要结合慢查询、查询频率、选择性、覆盖能力和写入成本综合判断，而不是“字段可能会查就建索引”。
