...
MVCC 的核心是让普通快照读读取某个时间点的数据版本，而不是直接阻塞最新数据行。InnoDB 通过隐藏字段、undo log 版本链和 Read View 判断某个版本对当前事务是否可见。

每行记录通常可以理解为带有两个隐藏信息：创建或最后修改该行版本的事务 ID，以及指向旧版本的回滚指针。事务执行快照读时，会根据 Read View 判断当前版本是否可见；不可见就沿 undo log 版本链向前找，直到找到可见版本或确认不存在。

因此普通 `SELECT` 在可重复读下通常不需要给记录加锁，也不会被其他事务的更新阻塞。代价是需要维护 undo 版本链，长事务会导致旧版本迟迟不能清理，进而膨胀 undo 空间并拖慢查询。

## Read View 里有哪些关键信息？

Read View 可以理解为一次快照读的“可见性规则”。面试中常提四个关键字段。

- `m_ids` ：生成 Read View 时仍然活跃的事务 ID 集合。
- `min_trx_id` ：活跃事务中最小的事务 ID。
- `max_trx_id` ：下一个将要分配的事务 ID。
- `creator_trx_id` ：创建该 Read View 的事务 ID。

判断版本可见性时，若版本事务 ID 小于 `min_trx_id`，说明它在快照前已提交，可见；若大于等于 `max_trx_id`，说明它在快照后才出现，不可见；若落在中间，需要看是否在活跃事务集合中，在集合内通常不可见，不在集合内说明已经提交，可见。

## RC 和 RR 下 Read View 的生成时机有什么不同？

读已提交，简称 RC，每次快照读都会生成新的 Read View。因此同一事务里两次普通查询可能看到其他事务新提交的数据，容易出现不可重复读。

可重复读，简称 RR，事务第一次执行快照读时生成 Read View，后续快照读复用同一个 Read View。因此同一事务中的普通查询结果通常保持一致。

但要注意：当前读不走快照版本，而是读取最新已提交版本并加锁，例如 `SELECT ... FOR UPDATE`、`UPDATE`、`DELETE`。所以在 RR 中，普通快照读和当前读可能看到不同结果。

## InnoDB 的 RR 是如何缓解幻读的？

幻读指同一事务按相同条件查询，两次结果集中出现了之前不存在的记录。InnoDB 在 RR 下用两类机制缓解。

- **快照读** ：通过 MVCC 和固定 Read View，让普通查询看不到后续事务插入的新版本。
- **当前读** ：通过 next-key lock，也就是记录锁加间隙锁，锁住命中的记录及其前后间隙，阻止其他事务在范围内插入新记录。

需要强调的是，MVCC 解决的是快照读的一致性；当前读要避免幻读，仍然依赖锁。索引是否命中也很关键，如果范围条件没有合适索引，锁范围可能扩大，性能和并发都会变差。
