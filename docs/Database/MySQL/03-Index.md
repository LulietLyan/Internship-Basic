...
索引下推，简称 ICP，是 MySQL 在使用二级索引扫描时，把一部分 `WHERE` 条件下推到存储引擎层提前判断。这样可以减少回表次数。

例如联合索引 `(name, age)`，查询条件是 `name LIKE '张%' AND age = 20`。如果只能用 `name` 做范围扫描，传统方式可能先拿到所有满足 `name` 前缀的主键再回表，然后由 Server 层判断 `age`。启用索引下推后，存储引擎在扫描索引时就能判断 `age`，不满足的记录不回表。

它的关键收益不是减少索引扫描，而是减少不必要的回表。判断是否生效可以看 `EXPLAIN` 的 `Extra` 中是否出现 `Using index condition`。

## 什么是覆盖索引？它为什么快？

覆盖索引指查询需要的列都能从某个索引中直接拿到，不需要回表读取聚簇索引记录。比如有联合索引 `(user_id, status, create_time)`，查询只返回这几个字段，就可能成为覆盖索引。

覆盖索引快的原因很直接：二级索引叶子节点已经包含索引列和主键值，如果查询列被索引覆盖，就省掉“二级索引查主键，再用主键查聚簇索引”的过程。

面试里常见坑点是：`SELECT *` 往往破坏覆盖索引；联合索引字段顺序既影响最左匹配，也影响能否覆盖查询列和排序列。
