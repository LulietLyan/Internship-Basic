...
## Redis 的 SDS 为什么不用 C 字符串？

SDS 是 Redis 的简单动态字符串。它相比 C 字符串多维护了长度、容量等元数据，因此获取字符串长度是 O(1)，拼接时也能根据剩余空间减少频繁扩容。

C 字符串用 `\0` 作为结束标记，不适合保存任意二进制数据；SDS 会记录真实长度，所以可以保存图片、压缩包、序列化对象等二进制内容。

面试中可以从三个角度答。

- **安全** ：拼接前可以检查容量，降低缓冲区溢出风险。
- **高效** ：长度 O(1)，扩容有预分配策略。
- **二进制安全** ：内容中可以包含 `\0`。

## Redis zset 为什么常用跳表？

zset 需要同时支持按成员查分数、按分数范围查询、排名查询等操作。Redis 常用字典加跳表实现：字典用于 O(1) 定位成员分数，跳表用于按分数有序遍历和范围查询。

跳表相比红黑树实现更简单，范围遍历天然友好，平均复杂度也能达到 O(logN)。在面试里可以强调：跳表并不是严格平衡树，而是通过多层随机索引达到概率上的平衡。

## Redis 哈希表渐进式 rehash 是什么？

Redis 字典扩容或收缩时不会一次性搬迁所有键值对，而是同时维护旧表和新表。后续每次读写、删除、定时任务都会顺手迁移一部分桶，这个过程叫渐进式 rehash。

这么做是为了避免单线程事件循环被一次性 rehash 阻塞太久。rehash 期间查询会同时查两张表，新增通常写入新表，直到旧表迁移完成再释放。

它体现了 Redis 的一个常见设计思路：把大任务拆成小步执行，控制单次事件循环延迟。
...
- **消息队列** ：可用 LPUSH + BRPOP 实现简单队列。注意两点缺陷：生产者需自行实现全局唯一 ID；无法以消费组形式消费，不支持多消费者组与已读进度管理
...
主要用于存储地理位置信息，并对存储的信息进行操作（如附近的人、打车距离）。底层由 Zset 实现，使用 GeoHash 编码将经纬度映射到 Zset 的权重分数，关键机制是「对二维地图做区间划分」和「对区间编码」。
...
简单动态字符串（SDS）是 String 的底层实现，相比 C 字符串有三点优势：

- **可存二进制** ：用 len 判断结束而非 `\0`，API 按二进制方式处理 buf，因此可存图片、压缩包等
- **O(1) 取长度** ：C 字符串要遍历，SDS 用 len 属性，复杂度 O(1)
- **安全、不溢出** ：拼接前会检查并自动扩容，不会发生缓冲区溢出
