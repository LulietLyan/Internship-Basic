...
## Redis 主从复制的全量同步和部分同步有什么区别？

全量同步通常发生在从节点第一次连接主节点、复制偏移差距过大或复制积压缓冲区无法覆盖缺失数据时。主节点生成 RDB 快照并发送给从节点，从节点加载快照后再接收增量命令。

部分同步依赖复制偏移量、主节点 run id 和复制积压缓冲区。从节点短暂断开后，如果主节点缓冲区仍保留缺失命令，就只补发增量数据，成本远低于全量同步。

面试中要补一句：Redis 主从复制默认是异步的，它提高读扩展和可用性，但不保证主从强一致。

## Sentinel 的主观下线和客观下线有什么区别？

主观下线是单个 Sentinel 认为某个主节点不可达。客观下线是多个 Sentinel 达成一定数量的判断后，认为主节点确实不可用，并触发故障转移。

这样设计是为了避免单个 Sentinel 网络抖动导致误判。故障转移时，Sentinel 会在从节点中选择一个更合适的节点提升为新主，并让其他从节点改为复制新主。

选择从节点时通常会考虑优先级、复制偏移量、run id 等因素。复制越新的从节点越有机会被选中，以减少数据丢失。

## Redis Cluster 为什么是 16384 个哈希槽？

Redis Cluster 把 key 映射到 16384 个 slot，再把 slot 分配给不同主节点。客户端根据 key 计算 slot，访问到错误节点时会收到 `MOVED` 或 `ASK` 重定向。

哈希槽让扩缩容更灵活：迁移的是 slot，而不是直接按节点数重新哈希所有 key。16384 是工程折中，既让 slot 数足够细，方便均衡迁移，又避免集群节点间维护过大的槽位元数据。

多 key 命令需要所有 key 落在同一个 slot。业务上可以用 hash tag，例如 `user:{100}:profile` 和 `user:{100}:order`，让大括号内的部分参与哈希，从而把相关 key 固定到同一槽位。

## Redis Cluster 脑裂会造成什么问题？如何缓解？

脑裂指网络分区后旧主节点仍然接受写入，同时集群另一侧已经选出新主。分区恢复后旧主降级，它在分区期间接受的写入可能丢失。

缓解方式包括合理配置 `min-replicas-to-write` 和 `min-replicas-max-lag`，让主节点在无法确认足够从节点在线时拒绝写入；同时要控制故障转移时间、监控复制延迟，并让关键业务具备幂等和补偿能力。

结论是：Redis Cluster 提供高可用和分片能力，但主从复制异步决定了它不能天然保证强一致写入。
