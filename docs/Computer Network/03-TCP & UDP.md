...
- 修改短连接为长连接
- 扩大可使用端口号的范围
- 客户端机器打开  **tcp_tw_reuse**  和  **tcp_timestamps** 选项： **tcp_tw_reuse**  调用 connect() 函数时，内核会随机找一个 time_wait 状态超过 1 秒的连接给新的连接复用。复用连接之后需要更新  **timestamps**  参数，当旧的 TCP 数据包到达时，根据时间戳判断是旧连接的数据可以舍弃
...
TCP 不关心应用层一次写入的是一条消息还是多条消息，它只把数据看成连续、有序的字节序列。发送端多次 `write` 的数据，接收端可能一次 `read` 读到；发送端一次 `write` 的数据，接收端也可能分多次读到。

因此 TCP 本身没有应用层消息边界，这也是粘包、拆包问题的根源。
...
- Nagle 算法可能把多个小包合并发送
- 接收方读取缓冲区的时机和大小不固定
- 发送的数据超过 MSS，会被拆成多个 TCP 段
- 应用层一次读取的数据长度和实际消息长度不一致
...
- 使用成熟的帧协议，例如 WebSocket Frame、HTTP/2 Frame、gRPC、Protobuf
...
简单记忆：流量控制看“对端能不能收下”；拥塞控制看“网络能不能承受”。

## TCP 拥塞控制有哪些阶段？

- **慢启动** ：拥塞窗口指数增长，用于快速探测可用带宽
- **拥塞避免** ：达到阈值后线性增长，避免过快压垮网络
- **快速重传** ：收到多个重复 ACK 后，不等超时就重传可能丢失的报文段
- **快速恢复** ：丢包后降低窗口，但不一定回到最小值，尽量保持吞吐

发生超时重传时，通常说明拥塞更严重，会把拥塞窗口降得更低；发生三次重复 ACK 时，通常认为网络仍在传输部分数据，可以进入快速重传和快速恢复。

## 超时重传和快速重传有什么区别？

- **超时重传** ：发送方在 RTO 时间内没有收到 ACK，就重传数据。它比较保守，等待时间更长，并且通常会显著降低拥塞窗口
- **快速重传** ：发送方收到多个重复 ACK，推测某个报文段丢失，于是在超时前重传。它能更快恢复丢包，减少等待

现代 TCP 还会结合 SACK，让接收方告诉发送方哪些数据块已经收到，从而只重传真正丢失的数据。

## SACK 和 D-SACK 分别解决什么问题？

- **SACK(Selective Acknowledgment)** ：选择性确认。接收方把已经收到的不连续数据块告诉发送方，发送方只重传缺失部分
- **D-SACK(Duplicate SACK)** ：重复 SACK。接收方告诉发送方某些数据被重复接收，可用于判断是数据包丢失、ACK 丢失、网络延迟还是发生了重复包

SACK 提升重传效率，D-SACK 更偏诊断和优化。

## TCP Keepalive 和 HTTP Keep-Alive 有什么区别？

- **TCP Keepalive** ：传输层探活机制，用于检测长时间空闲连接是否还存活。通常默认间隔很长，需要操作系统参数配置
- **HTTP Keep-Alive** ：应用层连接复用机制，让多个 HTTP 请求复用同一条 TCP 连接，减少建连和断连成本

两者名字相似，但目的不同：TCP Keepalive 是探活，HTTP Keep-Alive 是复用。

## 服务端出现大量 CLOSE_WAIT 通常说明什么？

`CLOSE_WAIT` 表示对端已经关闭连接，本机也已经收到 FIN 并回复 ACK，但本机应用还没有调用 `close()`。大量 `CLOSE_WAIT` 通常说明应用层没有正确释放连接。
...
- 查看进程持有的 socket 和 fd 数量
- 检查连接池归还逻辑
- 检查异常处理和 `finally close`
- 观察线程栈是否阻塞在 IO、锁或下游调用

## 为什么 UDP 仍然被大量使用？

UDP 不保证可靠、有序、不重复，也没有连接管理和拥塞控制，但它简单、延迟低、报文边界清晰。
...
- 音视频、直播、游戏等更关注实时性的场景
- 广播、组播
- QUIC 这类在应用层自行实现可靠传输、拥塞控制和加密的协议

“UDP 不可靠”不是全部结论，更准确地说是：UDP 把可靠性、顺序、重传和拥塞控制的选择权交给应用层。

## QUIC 如何在 UDP 上实现可靠传输？

QUIC 运行在 UDP 之上，但它自己实现了很多 TCP 才有的能力：

- **Packet Number** ：每个 QUIC Packet 有递增编号，用于确认和丢包检测
- **ACK Frame** ：接收方通过 ACK Frame 告诉发送方哪些包已经收到
- **Stream ID + Offset** ：标识不同流以及流内偏移，使多个流可以复用同一连接并独立重组
- **重传机制** ：丢失的数据根据 Stream Offset 重传，不要求重传完全相同的 UDP 包
- **流量控制** ：限制单个 Stream 和整个连接的数据量，避免接收方被打爆
- **拥塞控制** ：根据网络状态调整发送速率
- **TLS 1.3 加密** ：握手和加密与传输层结合，减少中间设备干扰

和 HTTP/2 over TCP 相比，QUIC 的多个 Stream 更独立。某个 Stream 丢包，通常只影响该 Stream，不会阻塞其他 Stream 向应用层交付。

## TCP 和 QUIC 的连接迁移有什么区别？

TCP 连接由四元组唯一标识：源 IP、源端口、目的 IP、目的端口。移动端从 WiFi 切到蜂窝网络后，源 IP 或端口可能变化，原 TCP 连接通常失效，需要重新建立连接。

QUIC 使用 Connection ID 标识连接。只要客户端和服务端仍保留连接上下文和密钥，即使 IP 地址变化，也可以通过 Connection ID 继续关联到原连接，从而降低移动网络切换带来的重连成本。

## 线上偶发连接超时，可能有哪些 TCP/UDP 层原因？

- DNS 解析到异常 IP 或解析耗时过长
- SYN 包丢失、SYN backlog 满、全连接队列满
- 服务端 fd、端口、连接数、内存等资源耗尽
- NAT、负载均衡、防火墙连接跟踪表满
- 丢包或重传严重，导致 RTO 增大
- 接收窗口太小或零窗口，发送方无法继续发送
- Path MTU Discovery 失败，大包被丢弃
- UDP 被限速、丢弃或被中间设备拦截
