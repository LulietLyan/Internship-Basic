...
原因：Linux 内核处理收到 TCP 报文的入口函数是 tcp_v4_rcv，在收到 TCP 报文后，会调用 __inet_lookup_skb 函数找到 TCP 报文所属 socket。这个函数首先查找连接建立状态的 socket，在没有命中的情况下，才会查找监听套接口。如果服务端没有调用 listen，就找不到监听该端口的 socket，最终找不到对应的 socket，内核就会调用 tcp_v4_send_reset 发送 RST 中止这个连接。
