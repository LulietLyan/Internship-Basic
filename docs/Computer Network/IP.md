...
- **A 类**：0.0.0.0 ~ 127.255.255.255，前 8 位为网络号，后 24 位为主机号，最大主机数 2^24-2
- **B 类**：128.0.0.0 ~ 191.255.255.255，前 16 位为网络号，后 16 位为主机号，最大主机数 2^16-2
- **C 类**：192.0.0.0 ~ 223.255.255.255，前 24 位为网络号，后 8 位为主机号，最大主机数 2^8-2
...
例如对 C 类地址 192.168.1.0 进行子网划分，使用子网掩码 255.255.255.192。C 类地址中前 24 位是网络号，最后 8 位是主机号，根据子网掩码可知**从 8 位主机号中借用 2 位作为子网号**，可以划分出 4 个子网。

## 公有 IP 地址与私有 IP 地址

在 A、B、C 分类地址中，实际上有分公有 IP 地址和私有 IP 地址。

**私有 IP 地址**允许组织内部的 IT 人员自己管理、自己分配，而且可以重复。平时我们办公室、家里、学校用的 IP 地址，一般都是私有 IP 地址。

**公有 IP 地址**是由 ICANN 组织管理，中文叫「互联网名称与数字地址分配机构」。公有 IP 地址基本上要在整个互联网范围内保持唯一。
...
IPv4 的地址紧缺，于是提出了一种**网络地址转换 NAT** 的方法，再次缓解了 IPv4 地址耗尽的问题。NAT 就是同个公司、家庭、教室内的主机对外部通信时，把私有 IP 地址转换成公有 IP 地址。

普通的 NAT 转换意义不大，因此引入了**网络地址与端口转换 NAPT**。NAPT 可以把 IP 地址 + 端口号一起进行转换，这样就用一个全球 IP 地址就可以了。
...
RARP 协议正好与 ARP 相反，它是**已知 MAC 地址求 IP 地址**。例如将打印机服务器等小型嵌入式设备接入到网络时就经常会用得到。通常这需要架设一台 RARP 服务器，在这个服务器上注册设备的 MAC 地址及其 IP 地址。

## DHCP 协议

DHCP 可以为设备动态获取 IP 地址，大大省去了配 IP 信息繁琐的过程。DHCP 客户端进程监听的是 68 端口号，DHCP 服务端进程监听的是 67 端口号。

**DHCP 工作流程**：
- **客户端发起 DHCP 发现报文(DHCP DISCOVER)**：由于客户端没有 IP 地址，也不知道 DHCP 服务器的地址，所以使用的是 **UDP 广播通信**。其使用的广播目的地址是 255.255.255.255(端口 67)并且使用 0.0.0.0(端口 68)作为源 IP 地址
...
如果租约的 DHCP IP 地址快过期了，客户端会向服务器发送 DHCP 请求报文，服务器如果同意继续租用，则用 DHCP ACK 报文进行应答，客户端就会延长租期。

为了解决 DHCP 服务器和客户端不在同一个局域网内的问题，出现了 **DHCP 中继代理**。有了 DHCP 中继代理以后，**对不同网段的 IP 地址分配也可以由一个 DHCP 服务器统一进行管理**。
...
ICMP 全称是 **Internet Control Message Protocol**，即 **互联网控制报文协议**。其主要功能有 **确认 IP 包是否成功送达目标地址、报告发送过程中 IP 包被废弃的原因和改善网络设置等**。
...
当某个 IP 包因为某种原因未能达到目标地址时，这个具体的原因将由 ICMP 负责通知。ICMP 报文会使用 IP 进行发送。

## IGMP 协议

IGMP 全称是 **Internet Group Management Protocol**，即 **因特网组管理协议**。IGMP 协议用于管理组播组，工作在主机(组播成员)和最后一跳路由器之间。

**IGMP 工作原理**：
- **常规查询与响应**：路由器会周期性发送目的地址为 `224.0.0.1`(表示同一网段内所有主机和路由器)的 IGMP 常规查询报文。主机收到查询后，会启动报告延迟计时器，计时器超时后发送 IGMP 成员关系报告报文。路由器收到主机的成员关系报文后，就会在 IGMP 路由表中加入该组播组，后续组播包到达路由器时，路由器会把数据包转发给该组播组的主机
- **离开组播组**：主机要离开组播组时，发送 IGMP 离组报文。路由器收到该报文后，会发送 IGMP 特定组查询报文，确认该网络是否还有该组播组的其他成员。如果没有其他成员，路由器就不再转发该组播组的数据包

**IGMP 特点**：
- IGMP 报文采用 IP 封装，IP 头部的协议号为 2
- IGMP 的 TTL 字段值通常为 1，因为 IGMP 是工作在主机与连接的路由器之间
- 默认情况下路由器不会转发组播包到连接中的主机，除非主机通过 IGMP 加入到组播组
...
- **IPv6 协议支持地址自动配置**，这是一种即插即用的机制。IPv6 节点通过地址自动配置得到 IPv6 地址和网关地址。IPv6 支持无状态地址自动配置和状态地址自动配置两种地址自动配置方式
- **IPv6 取消了首部校验和字段**，因为在数据链路层和传输层都会校验
- **IPv6 取消了分片/重新组装相关字段**，分片与重组是耗时的过程，IPv6 不允许在中间路由器进行分片与重组，这种操作只能在源与目标主机
- 在 IPv6 中加入了关于身份验证、数据一致性和保密性的内容

**IPv6 地址表示方法**：IPv6 地址长度是 128 位，是以每 16 位作为一组，每组用冒号「:」隔开。如果出现连续的 0 时还可以将这些 0 省略，并用两个冒号「::」隔开，但是一个 IP 地址中只允许出现一次两个连续的冒号。
