...
|---|---|---|---|
| 应用层、表示层、会话层 | 应用层 | 应用层 | HTTP、HTTPS、DNS、WebSocket、SMTP |
| 传输层 | 传输层 | 传输层 | TCP、UDP、QUIC(基于 UDP 实现) |
| 网络层 | 网际层 | 网络层 | IP、ICMP、路由器 |
| 数据链路层 | 网络接口层 | 数据链路层 | Ethernet、ARP、交换机、MAC |
| 物理层 | 网络接口层 | 物理层 | 网线、光纤、无线电信号 |

回答时重点不是死背层数，而是说清楚每层解决的问题：应用层定义业务报文格式，传输层解决进程到进程通信，网络层解决主机到主机路由，链路层解决同一链路内的帧传输，物理层解决比特流传输。
...
2. 传输层加 TCP/UDP 头，形成 Segment 或 Datagram，携带端口、序列号、校验等信息
3. 网络层加 IP 头，形成 Packet，携带源 IP、目的 IP、TTL 等信息
4. 数据链路层加帧头和帧尾，形成 Frame，携带源 MAC、目的 MAC、FCS 等信息
5. 物理层把帧转换为比特流在介质上传输
...
- **URL 解析**：浏览器判断协议、域名、端口、路径、查询参数；如果没有协议，可能先补全或走搜索
- **DNS 解析**：查浏览器缓存、系统缓存、hosts、本地 DNS，再递归/迭代查询权威 DNS
- **建立连接**：HTTP 走 TCP 三次握手；HTTPS 还要 TLS 握手；HTTP/3 基于 QUIC
- **发送请求**：构造请求行、请求头、空行、请求体；携带 Cookie、缓存条件、认证信息等
- **服务端处理**：反向代理、路由、业务逻辑、数据库或下游服务调用
- **返回响应**：状态码、响应头、响应体；可能命中缓存、压缩、分块传输、重定向
- **浏览器渲染**：解析 HTML 构建 DOM，解析 CSS 构建 CSSOM，执行 JavaScript，构建渲染树，布局、绘制、合成
- **加载子资源**：图片、CSS、JS、字体等资源继续触发 DNS、连接复用、缓存判断和请求

## 浏览器访问网站很慢，如何按网络分层排查？

可以从低层到高层，也可以按请求瀑布图逐段排查：

- **DNS 阶段**：解析是否慢，是否解析到错误 IP，是否被劫持或污染
- **TCP 阶段**：三次握手耗时、丢包、重传、SYN 队列是否满
- **TLS 阶段**：证书链是否过长，协议版本是否过旧，握手是否失败
- **HTTP 阶段**：是否重定向过多，缓存是否命中，状态码是否异常，响应体是否过大
- **服务端阶段**：应用耗时、数据库、缓存、下游 RPC、线程池、连接池是否瓶颈
- **前端阶段**：阻塞资源、JavaScript 执行、图片体积、首屏渲染、长任务

常用工具：

- `ping`：检查基本可达性和 RTT，但目标禁 ICMP 时不一定可靠
- `traceroute` / `tracert`：观察路由路径和大致故障跳点
- `nslookup` / `dig`：检查 DNS 解析
- `telnet` / `nc`：检查 TCP 端口连通性
- `curl -v`：观察 DNS、TCP、TLS、HTTP 状态码和响应头
- 浏览器 DevTools Network：查看瀑布图、缓存、资源大小和加载耗时

## 502、503、504 分别表示什么？

- **502 Bad Gateway**：网关或代理收到了上游的错误响应，例如 Nginx 连接后端失败、上游返回非法响应
- **503 Service Unavailable**：服务暂时不可用，可能是过载、维护、限流、健康检查失败
- **504 Gateway Timeout**：网关等待上游响应超时，常见于后端处理慢或网络链路异常

排查方向包括反向代理配置、上游服务存活、端口连通性、超时时间、负载均衡健康检查、应用日志和下游依赖耗时。
