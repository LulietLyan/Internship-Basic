...
HTTP 本身不会保存两次请求之间的状态。登录态、购物车、用户偏好等会话信息，通常通过 **客户端携带身份标识，服务端据此恢复状态** 来实现。

常见方式如下：

- **Cookie** ：服务端通过 `Set-Cookie` 把一小段键值对写入浏览器，浏览器后续访问同站点时自动携带 `Cookie` 请求头。常用于保存 `session_id`、语言、主题等小型状态
- **Session** ：服务端保存会话数据，客户端只保存 `session_id`。后续请求带上 `session_id` 后，服务端从内存、数据库或 Redis 中查找对应会话
- **Token/JWT** ：服务端签发令牌，客户端后续放到 `Authorization: Bearer <token>` 或 Cookie 中。服务端校验签名、过期时间和权限声明后识别用户
- **URL 重写** ：把会话 ID 拼到 URL 中，例如 `/cart?sessionid=xxx`，可用于禁用 Cookie 的场景，但安全性差，容易泄露到日志、Referer 或浏览器历史中
- **隐藏表单字段** ：把状态放进表单隐藏字段中，提交时带回服务端。适用范围很窄，主要用于表单流程
...
|---|---|---|---|
| 存储位置 | 客户端浏览器 | 服务端 | 客户端 |
| 服务端状态 | 不一定需要 | 需要保存会话 | 通常无状态校验 |
| 常见传输方式 | 自动放入 `Cookie` 请求头 | 通过 Cookie 携带 `session_id` | `Authorization` 请求头或 Cookie |
| 容量 | 单个通常约 4KB | 受服务端存储限制 | 不宜过大，JWT 常见约 1KB 左右 |
| 安全重点 | 防 XSS、CSRF、泄露 | 防 session_id 被盗、会话固定 | 防 XSS、令牌泄露、重放 |
| 适用场景 | 偏好设置、会话标识 | 传统 Web、后台系统、强注销 | 前后端分离、移动端、微服务认证 |

选择建议：

- 对强注销、权限变更实时生效要求高：优先 Session，服务端集中管理更方便
- 对跨服务共享身份、移动端、API 调用要求高：可用 Token/JWT，但要控制有效期并设计刷新和撤销机制
- 不要把密码、手机号、权限密钥等敏感信息明文放进 JWT payload，因为 payload 只是 Base64URL 编码，不是加密

## Cookie 的 HttpOnly、Secure、SameSite 有什么作用？

- **HttpOnly** ：禁止 JavaScript 读取 Cookie，降低 XSS 窃取 Cookie 的风险
- **Secure** ：只允许 Cookie 通过 HTTPS 发送，避免在 HTTP 明文链路中泄露
- **SameSite** ：限制跨站请求是否携带 Cookie，用于缓解 CSRF
    - `Strict`：完全禁止第三方站点携带
    - `Lax`：大多数跨站子请求不携带，顶级导航 GET 可携带
    - `None`：允许跨站携带，但必须配合 `Secure`

!!! note
    `HttpOnly` 不能阻止浏览器自动带 Cookie 发请求，所以它不能单独防 CSRF。CSRF 通常还需要 `SameSite`、CSRF Token、`Origin/Referer` 校验等手段。

## HTTP 常见请求方法及作用？

| 方法 | 语义 | 常见场景 |
|---|---|---|
| GET | 获取资源 | 查询页面、搜索、拉取列表 |
| POST | 提交数据，常用于创建资源或触发处理 | 表单提交、上传文件、创建订单 |
| PUT | 全量替换指定资源，不存在时可创建 | 更新 `/users/1` 的完整信息 |
| PATCH | 局部修改资源 | 修改邮箱、昵称等少数字段 |
| DELETE | 删除资源 | 删除用户、文章、评论 |
| HEAD | 获取响应头，不返回响应体 | 检查资源是否存在、文件大小、缓存元信息 |
| OPTIONS | 查询服务端支持的方法和跨域能力 | CORS 预检请求 |
| TRACE | 回显请求，主要用于诊断 | 现代服务通常禁用，避免安全风险 |
| CONNECT | 建立隧道 | HTTPS 代理 |

## PUT 和 POST 的区别？

- **POST** 通常作用于资源集合，例如 `POST /users`，表示由服务端创建一个新用户并分配 ID；多次提交可能创建多个资源，所以通常不幂等
- **PUT** 通常作用于明确资源，例如 `PUT /users/1`，表示把 1 号用户替换为请求体给出的完整状态；多次提交同一个请求，最终资源状态一致，所以通常幂等
- 简单记忆：POST 偏“提交给集合，由服务端决定结果”；PUT 偏“把指定资源更新成这个样子”

## PUT 和 PATCH 的区别？

- **PUT** 是全量更新，客户端应提交资源的完整新版本，遗漏字段可能被服务端理解为删除或置空
- **PATCH** 是局部更新，客户端只提交需要变更的字段
- **幂等性** ：PUT 通常幂等；PATCH 不一定幂等，取决于设计。例如“把用户名改成 Tom”幂等，“积分增加 10”不幂等

## 什么是幂等？HTTP 哪些方法通常是幂等的？

幂等指同一个请求执行一次和执行多次，对服务端资源状态的影响相同。网络中可能因为超时、重试、代理重放导致同一请求多次到达服务端，所以支付、下单、扣库存等接口必须重点设计幂等。
...
- 客户端提交唯一业务单号或 `Idempotency-Key`
- 服务端用唯一索引防重复写入
- 保存请求处理状态，重复请求直接返回第一次处理结果

## URL 编码的意义是什么？

URL 编码也叫百分号编码，会把 URL 中不安全或非 ASCII 字符转换为 `%` 加两位十六进制的形式，例如空格常编码为 `%20`，中文会按 UTF-8 字节序编码。

作用：

- **兼容性** ：URL 只能安全传输有限字符集，中文、空格等需要编码
- **避免歧义** ：`&`、`=`、`?`、`#` 等字符在 URL 中有特殊含义，作为普通数据时必须编码
- **标准化** ：让不同客户端、代理、服务端对 URL 的理解尽量一致

## URI、URL、URN 的区别？

- **URI** ：统一资源标识符，是总概念，用来标识一个资源
- **URL** ：统一资源定位符，是带访问方式和位置的 URI，例如 `https://example.com/index.html`
- **URN** ：统一资源名称，只强调名字，不强调位置，例如 `urn:isbn:0451450523`

可以理解为：URL 和 URN 都属于 URI；平时浏览器地址栏中最常见的是 URL。

## HTTP 请求报文由哪些部分组成？

HTTP/1.x 请求报文通常由 **请求行、请求头、空行、请求体** 组成。
...
- **请求行** ：包含请求方法、请求目标、HTTP 版本
- **请求头** ：携带主机、内容协商、认证、缓存、Cookie 等元信息
- **空行** ：标识请求头结束
- **请求体** ：POST、PUT、PATCH 等方法常携带 JSON、表单或文件数据

## HTTP 响应报文由哪些部分组成？

HTTP/1.x 响应报文通常由 **状态行、响应头、空行、响应体** 组成。
...
- **响应头** ：携带内容类型、长度、缓存策略、Cookie、服务器信息等
- **空行** ：标识响应头结束
- **响应体** ：返回 HTML、JSON、图片、文件等实际内容

## HTTP 缓存完整流程是什么？

![HTTP 缓存决策流程](../Figures/Computer%20Network/HTTP缓存决策流程.svg)

浏览器缓存通常先判断强缓存，再判断协商缓存：

1. **强缓存命中** ：浏览器直接使用本地缓存，不发请求。相关字段是 `Cache-Control` 和 `Expires`，其中 `Cache-Control` 优先级更高
2. **强缓存失效** ：浏览器向服务端发起条件请求，常见请求头是 `If-None-Match` 和 `If-Modified-Since`
3. **协商缓存命中** ：服务端返回 `304 Not Modified`，浏览器复用本地缓存
4. **协商缓存未命中** ：服务端返回 `200 OK` 和新资源，浏览器更新缓存副本

## Cache-Control 常见指令有哪些？

- `max-age=3600`：资源在 3600 秒内可直接使用强缓存
- `no-cache`：可以缓存，但使用前必须向服务端重新验证。它不是“不缓存”
- `no-store`：完全不允许缓存，请求和响应内容都不应被存储，适合敏感数据
- `public`：允许浏览器、代理、CDN 等共享缓存存储
- `private`：只允许用户浏览器缓存，不允许共享缓存保存
- `must-revalidate`：缓存过期后必须向源站重新验证，不能直接使用过期副本
...
- 分布式部署中要注意不同机器生成的 ETag 是否一致，否则会降低缓存命中率

## HTTP 长连接是什么？为什么需要长连接？

HTTP 长连接是指多个 HTTP 请求/响应复用同一条 TCP 连接。HTTP/1.0 默认短连接，需要显式使用 `Connection: keep-alive`；HTTP/1.1 默认长连接。
...
- 避免频繁进入 TCP 慢启动
- 减轻服务端连接创建和销毁压力
- 对同一页面加载 HTML、CSS、JS、图片等多个资源更友好

缺点：

- 长时间空闲连接会占用服务端资源
- 需要合理设置 keep-alive 超时和最大请求数
- HTTP/1.1 在同一连接上仍容易受队头阻塞影响

## HTTP/1.1 管道化是什么？为什么没有广泛使用？

管道化允许客户端在同一个 TCP 连接上连续发送多个请求，而不必等前一个响应返回后再发送下一个请求。

问题在于 HTTP/1.1 要求响应必须按请求顺序返回。如果第一个请求处理很慢，后面的响应即使已经准备好也不能先返回，因此仍存在队头阻塞。此外，中间代理和服务器支持不一致，错误处理复杂，所以现代浏览器基本没有依赖它，而是转向多连接、HTTP/2 多路复用和 HTTP/3。

## HTTP 长连接、管道化、多路复用有什么区别？

- **长连接** ：复用 TCP 连接，减少建连和断连成本
- **管道化** ：同一连接上连续发多个请求，但响应仍按顺序返回
...
- **Message** ：一次完整的 HTTP 请求或响应，由一个或多个 Frame 组成
- **Stream** ：一条逻辑双向字节流，承载一组请求/响应 Message，有唯一 Stream ID
- **Connection** ：底层 TCP 连接，一个连接中可以并发多个 Stream

HTTP/2 帧头固定 9 字节，包含长度、类型、标志位和 Stream ID。接收方根据 Stream ID 将乱序到达的帧重新组装成对应消息。

## HTTP/2 的 HPACK 如何压缩头部？

HTTP/1.x 每次请求都重复发送大量 Header，例如 `Cookie`、`User-Agent`、`Accept`。HTTP/2 使用 HPACK 压缩头部，核心包含：

- **静态表** ：预置常见头字段和伪头字段，例如 `:method`、`:path`、`:status`
- **动态表** ：连接过程中维护最近出现过的头字段，后续可用索引引用
- **Huffman 编码** ：进一步压缩字符串

这样重复 Header 不必每次完整传输，可以显著减少请求头体积。

## HTTP/2 Server Push 是什么？有什么问题？

Server Push 允许服务端在客户端明确请求前，主动推送可能需要的资源。例如客户端请求 `index.html`，服务端提前推送 `style.css` 和 `script.js`。

工作方式：

- 服务端通过 `PUSH_PROMISE` 告诉客户端将要推送某资源
- 推送资源使用服务端发起的偶数 Stream ID
- 客户端可以接受，也可以拒绝推送
...
- 服务端很难准确判断客户端是否已有缓存
- 推送错误会浪费带宽
- 与现代浏览器缓存、CDN、优先级调度配合复杂

因此实际工程中 Server Push 使用并不多，很多场景改用 `preload`、`prefetch` 或更精细的资源加载策略。
...
HTTP/3 使用 QUIC 替代 TCP。QUIC 基于 UDP，在应用层实现可靠传输、拥塞控制、流量控制、TLS 1.3 加密和连接迁移。

主要改进：

- **减少握手延迟** ：首次连接通常 1-RTT，恢复连接可使用 0-RTT
- **缓解队头阻塞** ：不同 Stream 独立传输，某个流丢包不阻塞其他流
- **连接迁移** ：用 Connection ID 标识连接，移动端从 WiFi 切到蜂窝网络时更容易保持连接
- **强制加密** ：QUIC 集成 TLS 1.3，大多数报文头会被认证，Body 会被加密

## QUIC 的 0-RTT 有什么风险？

0-RTT 允许客户端在恢复连接时直接发送应用数据，延迟很低，但它有 **重放攻击** 风险。攻击者可能复制一段 0-RTT 数据并重放给服务器。

因此：

- 0-RTT 适合 GET、查询等幂等请求
- 不适合支付、下单、扣库存等非幂等操作
- 服务端需要结合时间窗口、Token、幂等 Key 等机制降低重放风险

## 什么是 RTT？

RTT 全称是 Round-Trip Time，表示一个数据包从发送端发出，到发送端收到对端响应所经历的往返时间。HTTP/TCP/TLS 优化中经常用 RTT 衡量握手和请求延迟。

例如：

- TCP 三次握手通常需要 1 RTT 后才能发送应用数据
- TLS 1.2 完整握手通常需要额外 RTT
- TLS 1.3 和 QUIC 的优化重点之一就是减少 RTT 消耗
...
- TLS 1.3 主握手通常 1-RTT 完成，TLS 1.2 常见完整流程需要 2-RTT
- TLS 1.3 支持 0-RTT 恢复，但 0-RTT 存在重放风险，不适合非幂等请求
- TLS 1.3 更多握手内容被加密，减少中间人可观察的信息

## 什么是数字证书？证书链如何验证？

数字证书用于把 **域名/主体身份** 和 **公钥** 绑定起来，并由可信 CA 签名背书。证书通常包含版本号、序列号、签名算法、颁发者、有效期、使用者、Subject Public Key Info、Subject Alternative Name 等信息。

客户端验证证书链时会检查：

- 证书是否在有效期内
- 访问域名是否匹配证书的 `Subject Alternative Name`
- 证书链能否逐级追溯到浏览器或操作系统信任的根 CA
- 中间证书和根证书签名是否有效
- 证书是否被吊销
- 服务端是否持有证书对应私钥，TLS 握手中的签名可以证明这一点

## HTTPS 如何保证机密性、完整性和身份认证？

- **机密性** ：通过 TLS 协商会话密钥，后续使用对称加密保护 HTTP 数据
- **完整性** ：通过 MAC 或 AEAD 算法检测数据是否被篡改
- **身份认证** ：通过 CA 签发的证书证明服务端身份，客户端校验证书链和域名

HTTPS 不是简单地“用公钥加密所有数据”。实际流程通常是用非对称密码或 ECDHE 完成密钥协商，再使用更高效的对称加密传输数据。

## HTTPS 一定安全吗？

不一定。HTTPS 保护传输链路，但业务和终端仍可能出问题：

- 用户忽略证书告警，信任了伪造证书
- 设备被安装恶意根证书
- 服务端 TLS 配置过旧，使用弱加密套件
- 页面存在 XSS、CSRF、越权、SQL 注入等业务漏洞
- 用户访问的是钓鱼域名，即使该域名本身也有合法 HTTPS 证书

## Forward 和 Redirect 有什么区别？

- **Forward(服务端转发)** ：服务端内部把请求交给另一个资源处理，浏览器地址栏不变，只发生一次客户端请求
- **Redirect(重定向)** ：服务端返回 3xx 和 `Location`，浏览器重新发起新请求，地址栏会变化

常见选择：

- 同一系统内部资源跳转、保留请求上下文：Forward
- 登录后跳转、域名变更、资源永久迁移：Redirect

## 301 和 302 如何选择？

- **301 Moved Permanently** ：永久重定向，搜索引擎和浏览器可能长期缓存，适合域名迁移、资源永久换地址
- **302 Found** ：临时重定向，适合登录跳转、临时活动页、A/B 测试等
- 需要保持请求方法和请求体时，可关注 `307 Temporary Redirect` 和 `308 Permanent Redirect`，它们不会像一些历史实现那样把 POST 改成 GET

## CORS 是什么？简单请求和预检请求有什么区别？

CORS 是浏览器的跨源资源共享机制，用于控制一个源的页面能否读取另一个源的响应。它是浏览器安全策略，不是服务端之间通信的限制。

- **简单请求** ：满足方法和头部限制，例如 `GET`、`HEAD`、部分 `POST`，浏览器直接发请求，再根据响应头判断是否允许页面读取
- **预检请求** ：复杂请求会先发送 `OPTIONS`，询问服务端是否允许该来源、方法和请求头
...
| WebSocket | 全双工长连接 | 聊天、游戏、协同编辑、高频双向通信 |
