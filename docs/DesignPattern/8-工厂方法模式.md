...
简单工厂把创建逻辑集中在一个工厂类里，新增产品往往需要 **修改工厂**（见 [简单工厂模式](./1-简单工厂模式.md)）。大鸟以 **雷锋做好事** 类比：若只有一个「雷锋」包办一切，忙不过来；应 **增加更多雷锋**——每个具体工厂只负责创建一种产品，这就是 **工厂方法模式**。
...
> 实际上工厂方法模式也很好理解，就拿“手机 Phone”这个产品举例，手机是一个抽象产品，小米手机、华为手机、苹果手机是具体的产品实现，而不同品牌的手机在各自的生产厂家生产。
...
![工厂方法模式类图](../Figures/Design%20Pattern/8-工厂方法模式/工厂方法模式.png)
...
工厂方法模式使得每个工厂类的职责单一，每个工厂只负责创建一种产品。当创建对象涉及一系列复杂的初始化逻辑，而这些逻辑在不同的子类中可能有所不同时，可以使用工厂方法模式将这些初始化逻辑封装在子类的工厂中。这种模式遵循了开闭原则：对扩展开放（添加新产品只需要添加新工厂），对修改关闭（不需要修改现有代码）。在现有的工具、库中，工厂方法模式也有广泛的应用，比如：
