...
切换 Oracle / SQL Server 时，不应修改业务代码——抽象工厂创建 **一族相互匹配的产品**（如 AccessUser + AccessDepartment）。

## 什么是抽象工厂模式

抽象工厂模式也是一种创建型设计模式，提供了一个创建一系列相关或相互依赖对象的接口，而无需指定它们具体的类。这种模式通过提供一个接口来创建一系列相关的对象，客户端不需要知道具体创建了哪些类，只需要知道工厂接口和抽象产品类即可。
...
![抽象工厂模式类图](../Figures/Design%20Pattern/15-抽象工厂模式/抽象工厂模式.png)
...
> **类图说明**：抽象工厂模式的类图展示了抽象工厂、具体工厂、抽象产品和具体产品之间的关系。抽象工厂定义了创建产品的方法，具体工厂实现这些方法来创建一组相关的产品。
