...
![适配器模式类图](../Figures/Design%20Pattern/17-适配器模式/适配器模式.png)
...
- 在`.NET`中，`DataAdapter` 用于在数据源(如数据库)和 `DataSet` 之间建立适配器，将数据从数据源适配到 `DataSet` 中，以便在.NET应用程序中使用。`DataAdapter` 充当了适配器的角色，它封装了数据访问的细节，使得 `DataSet` 可以独立于数据源工作，实现了数据访问层与业务逻辑层的解耦。
...
外籍球员需要 **翻译** 才能与教练沟通——适配器将 **不兼容接口** 转换为客户端期望的接口。
class USB:
    def charge(self):
        pass
