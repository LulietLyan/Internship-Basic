...
简历投递需要 **复制** 多份，而非从零重写。原型模式通过 **克隆** 已有实例来创建新对象，避免重复初始化开销，尤其当创建成本较高时。
...
![原型模式类图](../Figures/Design%20Pattern/9-原型模式/原型模式.png)
...
- **浅拷贝（Shallow Copy）**：只复制对象本身，不复制对象内部的引用类型成员变量。浅拷贝后的对象和原对象共享引用类型的成员变量，修改其中一个对象的引用类型成员变量会影响另一个对象。
- **深拷贝（Deep Copy）**：不仅复制对象本身，还递归复制对象内部的所有引用类型成员变量。深拷贝后的对象和原对象完全独立，修改其中一个对象不会影响另一个对象。

例如，如果一个类包含一个List类型的成员变量，浅拷贝只会复制List的引用，两个对象共享同一个List；而深拷贝会创建一个新的List，并将原List中的所有元素复制到新List中。

原型模式在现有的很多语言中都有应用，比如以下几个经典例子：

- Java 提供了 `Object` 类的 `clone()` 方法，可以实现对象的浅拷贝。类需要实现 `Cloneable` 接口并重写 `clone()` 方法。如果需要深拷贝，需要手动实现或者使用序列化/反序列化的方式。

- 在 .NET 中，`ICloneable` 接口提供了 `Clone` 方法，可以用于实现对象的克隆。需要注意的是，`ICloneable`接口定义的是浅拷贝，如果需要深拷贝，需要自己实现或者使用序列化/反序列化的方式。
