...
有缓冲channel内部维护一个固定大小的环形队列，用`buf`指针指向缓冲区，`sendx`和`recvx`分别记录发送和接收的位置索引。这样设计能高效利用内存，避免数据搬移。

**两个等待队列`sendq和recvq`**

用来管理阻塞的goroutine。`sendq`存储因channel满而阻塞的发送者，`recvq`存储因channel空而阻塞的接收者。这些队列用双向链表实现，当条件满足时会唤醒对应的goroutine。

**互斥锁**

`hchan`内部有个mutex，所有的发送、接收操作都需要先获取锁，用来保证并发安全。虽然看起来可能影响性能，但Go的调度器做了优化，大多数情况下锁竞争并不激烈。
...
## 12. 实现细节补充（`hchan`、makechan、阻塞语义）

### 有缓冲 channel 何时仍会阻塞？

**有缓冲 ≠ 全程非阻塞**：当缓冲区**已满**时，发送仍会阻塞（或配合 `select`/`default` 非阻塞发送）；当缓冲区**为空**时，接收仍会阻塞。只有“缓冲区未满的发送”和“缓冲区有数据的接收”在对应条件下可以不阻塞。

### `hchan` 与 `sudog`

运行时通过 `runtime.hchan` 表示 channel；`sendq` / `recvq` 为等待队列，元素为 `sudog`（绑定等待中的 G 与数据指针等）。发送路径在可直达等待方时可能**直接内存拷贝**（不经环形缓冲），否则写入环形缓冲或 `gopark`。

### `makechan` 的三种内存布局（概念）

1. `size == 0`：仅分配 `hchan` 本体（无独立 `buf`）。
2. 元素不含指针：常为 **`hchan` 与 `buf` 一次连续分配**，缓存友好。
3. 元素含指针：`hchan` 与 `buf` 可能**分开分配**，便于 GC 扫描。

### 向 `nil` channel 发送/接收

对 `nil` channel 在**阻塞模式**下会永久挂起（`gopark`）；非阻塞配合 `select` 可立即返回“未选中”。
